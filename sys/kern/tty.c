...
tty_init(void)
{
    ...
    for (y = 0; y < VGA_HEIGHT; ++y) {
        for (x = 0; x < VGA_WIDTH; ++x) {
            ...
            vt_buffer[i] = VGA_Entry(' ', vt_color);
        }
    }
}

void
tty_setcolor(uint8_t color)
{
    vt_color = color;
}

void
tty_putentryat(unsigned char character, uint8_t color, size_t x, size_t y)
{
...
tty_putchar(char ch)
{
    ...
    tty_putentryat(uch, vt_color, vt_column, vt_row);
